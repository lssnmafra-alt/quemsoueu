  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#071a64] text-white">
      ...
      <main className="relative z-10 mx-auto max-w-[1500px] px-4 pb-10 pt-24 md:px-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[280px_1fr_460px]">
          <section className="rounded-3xl border-4 border-cyan-200/25 bg-[#082c7a]/80 p-3 shadow-2xl backdrop-blur-xl lg:sticky lg:top-24 lg:h-fit">
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-1">
              {categories.map((category) => {
                ...
                return <button key={category.id} type="button" onClick={() => { setSelectedCategoryId(category.id); setSelectedId(category.items[0]?.id || ''); }} className={cn('relative overflow-hidden rounded-2xl border-4 bg-white text-left transition-all hover:-translate-y-1', active ? 'border-yellow-300 ring-4 ring-yellow-300/20' : 'border-white/15')}><div className="relative aspect-square overflow-hidden bg-red-800 lg:aspect-[5/3]"><SafeStoreImage src={storeCardImageUrl(cover)} alt={category.name} className="h-full w-full object-cover" fallbackClassName="h-full w-full bg-gradient-to-br from-red-700 to-red-950" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/35 to-transparent p-3 pt-10"><p className="truncate text-base font-black uppercase text-white">{category.name}</p><p className="text-[10px] font-black text-yellow-200">{category.ownedCount}/{category.totalCount} liberados</p></div></div></button>;
              })}
            </div>
          </section>
...
function storeCardImageUrl(item?: StoreItem | null) { return item?.cardImageUrl || item?.imageUrl || ''; }
