...
const RARITY_STYLE: Record<CardRarity, CSSProperties> = {
  comum: { '--rarity-a': '#efe5d0', '--rarity-b': '#9f927e', '--rarity-glow': 'rgba(239,229,208,.42)' } as CSSProperties,
...
export default function CharacterImage({
  ...
  useEffect(() => {
    if (!isOfficial || typeof window === 'undefined') { setOfficialDeckEditorId(''); return; }
  ...
  const renderRarityFrame = (children: ReactNode) => (
    <div className={cn('qse-rarity-card relative overflow-visible', className)} data-rarity={rarity} style={RARITY_STYLE[rarity]} title={`${name} - ${CARD_RARITY_LABELS[rarity]}`}>
      <div className="absolute inset-[3.5%] z-0 rounded-[1.25rem] bg-[linear-gradient(180deg,var(--rarity-a),var(--rarity-b))] shadow-[0_0_22px_var(--rarity-glow)]" />
      <div className="absolute inset-[7%_7%_15%_7%] z-10 overflow-hidden rounded-[1rem] bg-slate-950">{children}</div>
      {!brokenFrame && <img src={getCardRarityFrameUrl(rarity)} alt="" aria-hidden="true" onError={() => setBrokenFrame(true)} className="pointer-events-none absolute inset-[-7%] z-20 h-[114%] w-[114%] object-fill drop-shadow-xl" referrerPolicy="no-referrer" />}
      {!shouldHideOfficialName && <span className="pointer-events-none absolute left-[17%] right-[17%] top-[7.8%] z-30 truncate rounded-full bg-slate-950/72 px-2 py-0.5 text-center text-[9px] font-black uppercase tracking-[0.16em] text-white shadow-lg">{CARD_RARITY_LABELS[rarity]}</span>}
      {rarityPicker}
    </div>
  );
