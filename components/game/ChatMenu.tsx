...
export default function ChatMenu({ roomId, me, players = [], collapsible = false }: { roomId: string, me: any, players?: any[], collapsible?: boolean }) {
  ...
  return (
    <div className={cn(
      'flex flex-col overflow-hidden transition-all pointer-events-auto',
      collapsible
        ? 'fixed inset-x-3 bottom-[calc(0.75rem+env(safe-area-inset-bottom))] top-[max(0.75rem,env(safe-area-inset-top))] z-[80] sm:absolute sm:inset-x-auto sm:bottom-4 sm:right-4 sm:top-20 sm:w-[390px] sm:h-[min(720px,calc(100dvh-7rem))]'
        : 'w-full h-full bg-slate-50/50',
    )}>
      <div className={cn(
        'flex flex-col flex-1 min-h-0 w-full overflow-hidden pointer-events-auto',
        collapsible ? 'bg-indigo-950/94 sm:bg-indigo-950/86 backdrop-blur-md rounded-3xl border border-indigo-700/70 shadow-2xl h-full relative' : 'h-full bg-white relative',
      )}>
        <div className={cn('px-4 py-3 border-b shrink-0 flex items-center justify-between', collapsible ? 'border-indigo-800/70 bg-indigo-950/70' : 'border-indigo-50 bg-indigo-50/10')}>
          <span className={cn('text-xs font-black flex items-center gap-2', collapsible ? 'text-indigo-100' : 'text-indigo-950')}>
            <Smile className={cn('w-5 h-5', collapsible ? 'text-indigo-300' : 'text-indigo-500')} /> CHAT
          </span>
          {collapsible && (
            <button onClick={toggle} className="text-indigo-200 hover:text-white bg-indigo-900/70 hover:bg-indigo-800 p-2 rounded-xl transition-all cursor-pointer" aria-label="Fechar chat">
          ...
          {visibleMessages.map((m) => {
            ...
            return (
              <div key={m.id} className="flex flex-col w-full items-start">
                {collapsible ? (
                  <div className="text-[14px] leading-snug w-full rounded-2xl border border-indigo-800/50 bg-indigo-900/35 px-3 py-2 shadow-sm">
                    <span className={cn('font-black tracking-wide mr-1.5', senderColor?.text || 'text-indigo-300')}>
                      {m.sender_name}:
                    </span>
                    <span className="text-indigo-50 font-medium break-words [overflow-wrap:anywhere]">{m.content}</span>
                  </div>
                ) : (
                  <div className="p-3 text-sm max-w-[90%] font-semibold shadow-sm inline-block leading-relaxed border bg-white text-indigo-950 border-indigo-100 rounded-3xl rounded-tl-none border-2">
                    ...
                    <span className="text-slate-700 break-words [overflow-wrap:anywhere]">{m.content}</span>
                  </div>
                )}
              </div>
            );
        ...
        <div className={cn('p-3 shrink-0 border-t', collapsible ? 'border-indigo-800/70 bg-indigo-950/80 pb-[max(0.75rem,env(safe-area-inset-bottom))]' : 'border-indigo-50 bg-white p-4')}>
          <form onSubmit={send} className="relative flex items-center">
            <input
              ...
              className={cn(
                'w-full min-w-0 rounded-2xl px-4 py-3.5 pr-12 text-base font-semibold transition-all shadow-inner focus:outline-none',
                collapsible
                  ? 'bg-indigo-900/60 border border-indigo-700 text-indigo-50 placeholder:text-indigo-300 focus:border-indigo-300 focus:bg-indigo-900'
            ...
            />
            <button type="submit" className={cn('absolute right-2 text-indigo-500 hover:text-indigo-400 p-2.5 transition-colors cursor-pointer rounded-xl', collapsible ? 'text-indigo-300 hover:text-white' : '')} aria-label="Enviar mensagem">
