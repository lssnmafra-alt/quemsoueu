...
export default function RoomFinished({ room, players, me, isAdmin, leaveRoom }: any) {
  ...
  useEffect(() => {
    ...
    const loadEvents = async () => {
      try {
        const { data, error } = await supabaseGame.from('match_events').select('*').eq('room_id', room.id).order('created_at', { ascending: true });
        if (error) {
  ...
  const roomLeader = useMemo(() => players.find((player: any) => player.user_id === room.admin_id) || players.find((player: any) => player.is_admin), [players, room.admin_id]);
  ...
  const statsByPlayer = useMemo(() => {
    ...
    for (const event of events) {
      ...
      if (event.event_type === 'player_eliminated' && event.actor_player_id) getStats(event.actor_player_id).eliminations += 1;
      if ((event.event_type === 'timeout_warning' || event.event_type === 'timeout_eliminated') && event.target_player_id) getStats(event.target_player_id).timeouts += 1;
    }
    return map;
  }, [events]);
  ...
  const ranking = useMemo(() => {
    ...
    return [...players].sort((a: any, b: any) => {
      ...
      return (b.lives || 0) - (a.lives || 0) || (a.missed_turns || 0) - (b.missed_turns || 0) || (a.play_order ?? 999) - (b.play_order ?? 999) || String(a.nickname || '').localeCompare(String(b.nickname || ''));
    });
  ...
  const resetGame = async () => {
    ...
    for (const p of players) await supabaseGame.from('room_players').update({ lives: 0, is_eliminated: false, missed_turns: 0, play_order: null }).eq('id', p.id);
  ...
  const claimLeadership = useCallback(async () => {
    ...
    try {
      let query = supabaseGame.from('rooms').update({ admin_id: me.user_id }).eq('id', room.id).eq('status', 'FINISHED');
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setHandoffSeconds((current) => Math.max(0, current - 1)), 1000);
    return () => window.clearInterval(timer);
      ...
      <div className="max-w-3xl w-full p-4 md:p-10 bg-white border-4 border-indigo-100 shadow-xl relative z-10 rounded-3xl">
        ...
        <div className="mb-6 rounded-3xl border-4 border-indigo-50 bg-indigo-50/40 p-4 text-left">
          <div className="mb-3 flex items-center justify-between gap-3 border-b-2 border-indigo-100 pb-3">
            <h3 className="text-sm font-black uppercase tracking-widest text-indigo-950 flex items-center gap-2"><Medal className="w-5 h-5 text-amber-500" /> Ranking final</h3>
          ...
          <div className="space-y-2">
            {ranking.map((p: any, index: number) => {
              ...
              return (
                <div key={p.id} className={cn('flex items-center gap-3 rounded-2xl border-2 bg-white px-3 py-2 shadow-sm', isWinner ? cn(p.color?.border || 'border-amber-300', p.color?.lightBgc || 'bg-amber-50') : 'border-indigo-100', isOut && !isWinner && 'opacity-70 grayscale')}>
                  <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center text-sm font-black shrink-0', isWinner ? 'bg-amber-400 text-amber-950' : 'bg-indigo-50 text-indigo-700')}>{positionLabel(index)}</div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] font-bold uppercase text-slate-500">{isWinner ? 'Campeao' : isOut ? `Eliminado${eliminatedAt !== undefined ? ` #${eliminatedAt + 1}` : ''}` : 'Sobreviveu'} · vidas: {Math.max(0, p.lives || 0)} · faltas: {p.missed_turns || 0}</p>
                  </div>
        ...
        <div className="mb-6 rounded-3xl border-4 border-indigo-50 bg-white p-4 text-left">
          <div className="mb-3 flex items-center gap-2 border-b-2 border-indigo-50 pb-3"><Target className="w-5 h-5 text-indigo-500" /><h3 className="text-sm font-black uppercase tracking-widest text-indigo-950">Estatisticas</h3></div>
          <div className="grid gap-2 md:grid-cols-2">
            {ranking.map((p: any) => {
              const stats = statsByPlayer.get(p.id) || { hits: 0, misses: 0, eliminations: 0, timeouts: 0, turns: 0 };
              return <div key={p.id} className="rounded-2xl border-2 border-indigo-50 bg-indigo-50/30 px-3 py-2"><p className="text-xs font-black text-indigo-950 truncate">{p.nickname}</p><div className="mt-1 grid grid-cols-5 gap-1 text-center text-[10px] font-black uppercase text-slate-500"><span><Target className="mx-auto mb-0.5 w-3.5 h-3.5 text-emerald-500" />{stats.hits}</span><span><XCircle className="mx-auto mb-0.5 w-3.5 h-3.5 text-rose-500" />{stats.misses}</span><span><Skull className="mx-auto mb-0.5 w-3.5 h-3.5 text-slate-500" />{stats.eliminations}</span><span><Clock className="mx-auto mb-0.5 w-3.5 h-3.5 text-amber-500" />{stats.timeouts}</span><span><Medal className="mx-auto mb-0.5 w-3.5 h-3.5 text-indigo-500" />{stats.turns}</span></div></div>;
            })}
          </div>
        </div>

        <div className="mb-8 rounded-3xl border-4 border-indigo-50 bg-slate-50 p-4 text-left">
          <div className="mb-3 flex items-center justify-between gap-2 border-b-2 border-indigo-50 pb-3"><h3 className="text-sm font-black uppercase tracking-widest text-indigo-950 flex items-center gap-2"><ScrollText className="w-5 h-5 text-indigo-500" /> Historico de eliminacoes</h3><span className="text-[10px] font-black uppercase text-slate-400">{eliminationEvents.length}</span></div>
          {!eventsLoaded ? <p className="rounded-2xl bg-white px-4 py-3 text-xs font-bold text-slate-500">Carregando historico...</p> : eliminationEvents.length === 0 ? <p className="rounded-2xl bg-white px-4 py-3 text-xs font-bold text-slate-500">Historico detalhado indisponivel para esta partida.</p> : <div className="space-y-2">{eliminationEvents.map((event: any, index: number) => { const characterName = event.metadata?.target_name ? ` usando ${event.metadata.target_name}` : ''; const actor = event.actor_player_id ? playerName(event.actor_player_id) : 'Timeout'; const target = playerName(event.target_player_id); return <div key={event.id || `${event.target_player_id}-${index}`} className="rounded-2xl border-2 border-slate-100 bg-white px-4 py-3 text-xs font-bold text-slate-700"><span className="font-black text-indigo-600">#{index + 1}</span> {actor} eliminou {target}{characterName}.</div>; })}</div>}
        </div>

        <div className="space-y-4 max-w-sm mx-auto">
          {canReset ? <><div className="rounded-2xl border-2 border-emerald-100 bg-emerald-50 px-4 py-3 text-xs font-black uppercase tracking-wider text-emerald-800">{localLeadership ? 'Você assumiu a liderança da sala.' : 'Você lidera a sala.'}</div><Button onClick={resetGame} className="w-full h-14 text-sm font-black uppercase tracking-wider btn-squishy-green text-white cursor-pointer select-none">Jogar Novamente</Button></> : <div className="rounded-2xl border-2 border-indigo-100 bg-indigo-50 px-4 py-4 text-indigo-950"><div className="mb-2 flex items-center justify-center gap-2 text-xs font-black uppercase tracking-wider"><UserPlus className="w-4 h-4 text-indigo-500" /> Aguardando {leaderName} reiniciar</div><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-white border-2 border-indigo-100 text-2xl font-black font-mono text-indigo-700 shadow-sm">{claimingLeadership ? '...' : handoffSeconds}</div><p className="mt-2 text-[10px] font-bold uppercase text-indigo-500">Se o líder não agir, a liderança passa automaticamente.</p></div>}
          <Button onClick={leaveRoom} variant="outline" className="w-full h-14 text-sm font-black uppercase btn-squishy-white cursor-pointer select-none">Voltar para o Lobby</Button>
        </div>
      </div>
    </div>
  );
