...
create table if not exists public.avatar_animations (
  ...
  constraint avatar_animations_unique_event unique (avatar_skin_id, event_type, animation_key)
);
...
insert into public.avatar_animations (avatar_skin_id, event_type, animation_key, loop, sort_order)
select s.id, v.event_type, concat(s.r2_prefix, 'Animacao/', s.avatar_key, v.suffix, '.mp4'), v.loop, v.sort_order
...
cross join (
  values
    ...
    ('victory', '-2', false, 30),
    ('defeat', '-3', false, 40)
) as v(event_type, suffix, loop, sort_order)
where s.skin_code = 'skin-1'
on conflict (avatar_skin_id, event_type, animation_key) do update set
  loop = excluded.loop,
  sort_order = excluded.sort_order,
  is_active = true,
  updated_at = now();